# D&D 2024 工具站 · 总体项目文档(2026-09-11)

> 给 AI 助手与新电脑接手的人阅读:这是什么、怎么用、怎么改、怎么重建合集、怎么部署、有哪些坑。
> 最近更新:2026-09-11(游侠/圣武士 1 级施法修复+吟游诗人/术士改 2024 准备制;修人类专长「详情」死按钮;本地记忆 7 天过期+欢迎页清空按钮;文档改为带日期标题、更新前先做带日期备份)。
...
| **物品卡**(魔法物品卡) | `魔法物品卡/index.html` | 同款魔法物品卡排版站。数据 `data/items.js`(417 条),支持消耗品/充能/自定义卡 |
...
- **始终准备的法术不占位(核心规则)**:凡"始终准备"的法术(种族授予/德鲁伊语/宿敌/圣武斩/子职常备表等,见 `alwaysPreparedSpells()`)自动拥有、**不计入戏法数与准备法术数、不占用可选上限**(原文:"如果某职业的其他特性给了你始终准备着的法术,这些法术不计入你以此法准备的法术数量")。实现:法术列表中此类法术标「常备·不占位」徽章且禁止手选(toggleSpell 守卫);所有计数(已选/上限)一律剔除常备名;各职业准备数量一律查官方"准备法术"表(`PREPARED_TABLES`/`HALF_PREPARED`/`SUBCLASS_PREPARED`),不再用"施法属性调整值+等级"公式(该公式与官方表不等价,曾致 1 级德鲁伊只给 3 个,官方表为 4)。
...
**2026-09-14(二)·战斗风格替代项+打印跨页**:
- **P2-6 遗留·战斗风格替代项**(原"受祝福的勇士/德鲁伊教战士"待办):战斗风格选择卡新增职业专属替代选项——**游侠→「德鲁伊教战士」**(替代战斗风格,习得 2 道自选德鲁伊戏法,感知施法,每级可换 1 个)、**圣武士→「受祝福的勇士」**(替代战斗风格,习得 2 道自选牧师戏法,魅力施法)。选中后:戏法计数 +2(spellCantripCount);法术可选列表追加对应职业的戏法(classSpellList extraCantripClass);提示语照常显示。实测:2 级游侠选德鲁伊教战士→戏法 0→2、法术列表出现德鲁伊伎俩等德鲁伊戏法、可正常选满 2 个 ✓。
- **打印截断修复**:用户反馈"打印分成两页少了很多卡片"——`@media print` 中 `.sheet pre{break-inside:avoid}` 导致超一页高的 pre 块(职业特性长列表/法术详情)被浏览器整体截断丢弃。改为 `break-inside:auto`(允许自然跨页);table/bigstat 保留 avoid(小元素不拆分)。

**2026-09-14(一)·第 5 轮规则清单修复**(对照 `问题清单归档/人物卡/规则内容问题清单(2026-09-14)`,15 项;修改日志在同目录):
- **P0-1/P0-2 兼职施法模型重构**:新增 `primaryCaster()`(主职业优先→第一个施法兼职,含 EK/诡术师);`casterLevelForClass` 计入 EK/诡术师 floor(职业等级/3)(主/兼职两侧传子职);`slotsStruct` 成为唯一法术位口径(纯子职施法者用 SUBCLASS_CASTER_SLOTS[职业等级],否则 totalCasterLevel 合并查全施法表;魔契师池独立),`multiclassSpellSlotsText` 改由其生成;`maxSpellLevel` 遍历主+兼职取最高(subCasterMax 按 3/7/13/19 职业等级断点);isSpellcaster/getSpellStat/classSpellList/两个计数函数全部接入 primaryCaster —— 主职业非施法者+兼职法师等场景彻底修复。
- **P0-3/P0-4 超魔法与战技进度**:超魔法 2 级 2 个+10/17 级各 2(满级 6);战技 3 级 3 个+7/10/15 级各 2(满级 9)。
- **P1-5~8 子职常备表**:月亮/海洋/龙族 3 级补齐(月华之光/云雾术/造风术/龙息术);**大地结社改为四地形单选**(EARTH_TERRAIN 荒漠/极地/温带/热带各 3 道,子职卡 UI 选 S.druidTerrain,换子职/清空时重置;官方为长休可换)。
- **P1-9**:吟游诗人激励/游侠宿敌/术士先天术法 level1 文案按 2024 原文改写;**牧师新增「圣职(二选一)」UI**(保护者=军用武器+重甲计入护甲武器栏;奇术使=戏法+1)。
- **P1-10**:19 级追加「通用专长」选项;`finalAbi` 属性上限动态化(被传奇恩惠提升过的属性→30,其余 20)。
- **P2-11~14/P3-15**:战士 11/20 额外攻击描述改三次/四次;精灵「敏锐感官」(洞悉/察觉/求生)与人类「技能」(18 选 1)新增选择 UI 并计入角色卡技能栏与被动察觉;噩梦缠身者标注「VRGtR 扩展」(保留);信息板 EK 环阶改 subCasterMax 同口径;aasimo 键名改 aasimar。

**2026-09-13(二)·常备法术不占位+全职业官方准备表**(用户紧急反馈:上轮 P2-7 给德鲁伊加常备"动物交谈"后,1 级德鲁伊只能选 3 个一环法术,应为 4——常备不计入准备数):
- **全施法者准备数改官方表**:从 5echm 抓 吟游诗人/牧师/德鲁伊/术士/法师 职业表"准备法术"列建 `PREPARED_TABLES`(诗人=牧师=德鲁伊同表,1级=4…20级=22;术士 1级=2;法师独立至 20级=25),`spellTotalCount()` 弃用"施法属性调整值+等级"公式(与官方表不等价);法师法术书提示的"当前可准备"同步查表。
- **常备不占位全链路**:renderSpellList 对常备法术显示「常备·不占位」徽章+「✓ 已常备」(无选择按钮);toggleSpell 守卫拒绝手选常备(toast 说明);已选计数/上限判定/顶部计数条一律剔除常备名(`alwaysSet` 过滤);提示语加"常备法术自动拥有,不占戏法与准备位"。旧存档中已选的常备法术自动不再计数(显示归入常备列表)。
- **全量审查常备来源**:种族授予(grantedSpells)/德鲁伊语·动物交谈/游侠宿敌·猎人印记/圣武士圣武斩·至圣斩/22 子职常备表——全部经 alwaysSet 路径统一豁免,魔法学徒自选不受影响。规则已写入 §六 技术要点。

**2026-09-13(一)·规则内容问题清单修复批次**(对照 `问题清单归档/人物卡/规则内容问题清单(2026-09-13)`,修改日志在同目录):
- **P0-1**(已随 09-12 去重修复,实测旧行残留 0);**P0-2** 半施法者准备数改官方表 `HALF_PREPARED`(抓自 5echm 圣武士/游侠职业表"准备法术"列,两表相同,9级=9 ✓);**P0-3** 子职施法者法术位改官方表 `SUBCLASS_CASTER_SLOTS`(3:[2]/4-6:[3]/7-12:[4,2]/13-15:[4,3,2]/16-18:[4,3,3]/19-20:[4,3,3,1]),spellSlotsText/slotsStruct/multiclassSpellSlotsText 三处收敛,EK19 幻影 5-9 环位消除;**P1-4** 子职准备数改 `SUBCLASS_PREPARED`(3-20 官方表);**P1-5** MULTICLASS_PROF 五处按"作为兼职角色"原文订正(bard+乐器-cleric/druid/rogue 简易或军用武器-fighter 语义说明)。
- **P2-6 德鲁伊原初职能**:职业卡 术师/卫士 单选(S.druidOrder);术师→德鲁伊戏法+1(1级=3 ✓);卫士→角色卡护甲/武器行追加;pickClass/resetAll 清状态。待办:受祝福的勇士/德鲁伊教战士/武器精通选择。**P2-7**:alwaysPreparedSpells 增 德鲁伊语→动物交谈、圣武斩→至圣斩;新增 alwaysPreparedNotes() 在总览渲染无位施展注记(猎人印记×熟练/长休、至圣斩1次/长休、法师18法术精通、法师20招牌法术)。

**2026-09-12(四)·木精灵种族法术累加修复**:grantedSpells() 木精灵分支只写了 3级大步奔行/5级行动无踪,漏掉 1 级即有的德鲁伊伎俩——表现为"3 级替换掉了戏法"。修为累加式(1级德鲁伊伎俩,3级+大步奔行,5级+行动无踪)。同类排查:卓尔(舞光术/3妖火/5黑暗术)、高等精灵(魔法伎俩/3侦测魔法/5迷踪步)、提夫兰三种遗赠、侏儒/阿斯莫均为正确累加,无同类问题。

**2026-09-12(三)·法术书导入导出代码化**:应用户要求,法术书导入/导出全部改为 DND1 字母代码(与人物卡同一编码):「📤 导出代码」弹窗显示 `DND1:<base36…>`(按当前排序)+复制按钮(showCodeModal 轻量弹窗,clipboard 失败退化为全选);「📥 导入代码」prompt 粘贴解析按序放入(合并原「🧙 角色法术代码」按钮);**移除** .md 文件导出/导入与 file input/exportSelection-md/importFromFile(旧导出的 .md 文件不再支持)。人物卡总览指引文案同步改为「📥 导入代码」。

**2026-09-12(二)·施法能力判定综合化**:用户指出野蛮人可经起源专长获得施法——`isSpellcaster()` 不再只看职业,新增 `magicInitiateInfo()` 检测魔法学徒专长(来源:背景 侍僧/向导/智者 的「魔法学徒(牧师/德鲁伊/法师)」、人类专长「魔法学徒 Magic Initiate」默认法师列表、ASI 专长,两种命名格式兼容):非职业施法+MI → 法术步骤出现,2 戏法+1 一环(对应列表,maxSpellLevel=1),提示与「法术总览」走 `getSpellStat()`(牧师/德鲁伊=感知、法师=智力),总览法术位行显示"无(魔法学徒…免费施展一次)";pickBg/pickSpeciesChoice 后 `renderStepper()` 即时刷新步骤条。连带修复:renderStepper 过滤条件原为 `curClass().spell`(连奥法骑士都没算,与 go() 的 isSpellcaster 口径不一致)→ 统一 isSpellcaster;人类「健壮 Tough」HP 判定原 `==="健壮"` 永假(存储名带英文后缀)→ 改前缀匹配,人类健壮 HP+2×等级 现已生效。

**2026-09-12(一)·角色卡法术信息去重**:用户反馈"每一页都有法术选择的部分"——上轮加「法术总览」后,职业特性区块仍保留旧的施法属性/法术位/已选戏法/已选法术行,打印分页后法术内容散布多处。修:职业区块只留一行「施法信息与法术清单: 见下方『法术总览』」,施法汇总唯一化(总览 1 次+法术详情 1 次);@media print 补隐藏 .sheet 内按钮(复制代码按钮不上打印件)。

**2026-09-11(三)·法术总览+跨站法术代码**:
- **子职常备法术数据**(修 P2-4 之子集):从 5echm 抓取 22 个子职的「等级|法术」表注入 `SUBCLASS_SPELLS`(牧师4领域/德鲁伊4结社/圣武士4誓言/游侠2/术士3术法/魔契师4宗主/魅心学院手补/星辰结社手补;驯兽师·猎人·狂野术法·舞蹈·逸闻·勇气按 2024 规则无常备表)。⚠️ 抓取坑:圣武士表等级列是 `<th>` 需 td,th 同取;法术名须剥离粘连英文。源:`_subclass_spells.json`+`_inject_subclass_spells.js` 可重跑。
- **人物卡「法术总览」区块**(renderSheet):施法属性/法术攻击加值/法术豁免DC/法术位(含合计数)/【始终准备的法术】(种族授予+游侠宿敌·猎人印记+子职表按职业等级解锁,每项标来源,与自选去重)/【自选法术】(戏法+已准备)。
- **跨站法术代码**:buildSpellCode 生成 `DND1:<base36索引,…>`(两站共用同一份 391 条数据,已脚本验证逐位一致——改任一数据文件必须同步另一个!);人物卡总览区显示代码+复制按钮(clipboard 失败弹手动复制框);法术书顶栏「🧙 角色法术代码」prompt 粘贴解析,按顺序放入法术(已选跳过,非法格式明确报错)。与旧 A2 postMessage 通道并存。

**2026-09-11(二)·人物卡全局功能三件套**:
- **顶栏全局操作**:header 新增常驻按钮「⬆ 升级 / 📤 导出进度 / 📥 导入进度 / 🗑 清空记录」——每一页都可用(打印仍隐藏);原欢迎页清空按钮移除(顶栏取代)。
- **进度导出/导入**:exportProgress 生成 .md(可读摘要:角色/职业等级/种族背景/技能戏法法术 + ```json 机器块 dnd2024-character/v1 含 S 与表单输入);importProgressFile 解析后 Object.assign(S)+applyInputs+renderEverything+跳回原步骤。导入正则锚定行首 `/^```json…/m`(说明文字不可再含围栏字样,否则先匹配说明行——已踩坑修复)。⚠️ IAB 内置浏览器 FileReader 返回 null 无法实测文件导入,用桩验证;正常浏览器不受影响(法术书/物品卡导入同机制早已上线)。
- **升级预览**:openLevelUp 弹窗列出 主职业/各兼职(升级对象)+「兼职新职业」(canMulticlass 属性校验);previewLevelUp/previewMulticlass 用「快照→虚拟升级→diff→还原」计算增益:该级新职业特性(CLASS_FEATURES,子职特性按已选子职展开)、属性提升/传奇恩惠、法术位差值(slotsStruct 与 multiclassSpellSlotsText 同规则:全施法合并/半施法/魔契师独立位+玄奥秘法,减法呈现如「+1 一环(共4)」)、准备/已知法术与戏法差值、魔能祈唤数、熟练加值跳档;确认后 applyLevelUp(+1 级,3 级自动跳去选子职)。差值口径=下一级表减上一级表(用户示例:术士2→3 显示「+1 一环(共4)、+2 二环(共2)」已实测一致)。

**2026-09-11(一)·人物卡规则与交互三修**(对照 5echm 玩家手册2024 各职业表核实):
- **游侠/圣武士 1-2 级施法修复**:2024 版半施法者 1 级即有施法(2 准备法术、2 个一环位)。修 `maxSpellLevelByClass`(paladin/ranger 不再对 lv≤2 返回 0)与 `HALF_CASTER_SLOTS`(1 级改 [2,0,0,0,0])——此前 1-2 级选不了法术且角色表法术位为空。
- **吟游诗人/术士改 2024 准备制**:2024 两职业为「准备法术」(数量=施法属性调整值+等级,每日可换),工具原按 2014 固定已知表(KNOWN_SPELLS)。CLASSES 两条目 `spellsKnown` → `prepSpells`,自动走"准备 N 个法术"分支;KNOWN_SPELLS 仅剩魔契师使用。全职业 1 级表逐一对表核实(魔契师/法师/牧师/德鲁伊无问题;法师书内/准备数上轮已区分提示)。准备数公式补 `Math.max(1,sm+lv)` 下限(低属性时不为 0/负)。
- **人类专长「详情」死按钮修复**:起源专长(ORIGIN_FEATS)名称带英文后缀(「警戒 Alert」),与一般专长表(「警戒」)不同名,`showFeatDetailByName` 按 'general' 查表失配后静默返回。改为依次查 起源→一般→传奇 三表。已扫描全部 onclick 处理器:无其它死按钮(print 为 window.print)。
- **本地记忆 7 天过期**:saveState 写入 `savedAt` 时间戳,loadState 超过 7 天(STATE_TTL_MS)自动清除按无记录处理。
- **欢迎页新增「🗑 清空记录,从头开始」按钮**(confirmReset→resetAll;resetAll 补清 `S.gearChoice`)。
- 文档流程:更新前先备份上一版为 `DMsBestFriend/项目文档_备份_YYYY-MM-DD.md`(标题带日期),两份文档标题均标注当前日期。

**2026-09-05·归档清单+四项新需求**:
